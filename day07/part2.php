...
class Item {
    ...
    function calculate_size($current) {
		...
		array_push($sizes, $current->size);
    }

	calculate_size($tree);
	$totalSpace = max($sizes);
    $freeSpace = 70000000 - $totalSpace;
    $requiredSpace = 30000000 - $freeSpace;
    echo "Total space: $totalSpace , Free space: $freeSpace, Required space: $requiredSpace\n";
    $candidates = array_filter($sizes, fn ($v) => $v >= $requiredSpace);
    $result = min($candidates);
	echo "The result for part 2 is: $result\n";
	print_r($sizes);
    echo "DONE\n";
?>